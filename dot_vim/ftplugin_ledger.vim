...
" If enabled this will show on the matches which are highest in the hierarchy
if !exists('g:ledger_highest_level_match')
  let g:ledger_highest_level_match = 0
endif
...
function! LedgerComplete(findstart, base) "{{{1
  if a:findstart
    ...
    if b:compl_context == 'account' "{{{2 (account)
      ...
      " my edits, don't always call insert
      if ! g:ledger_highest_level_match
...
function! LedgerFindInTree(tree, levels) "{{{1
  ...
  for branch in branches
    ...
    if !empty(nextlvls)
      if g:ledger_highest_level_match
        " line below removes lower hierarchy results
        call remove(results, len(results)-1)
      endif
...
" my edits
nnoremap <silent> glc :call LedgerToggleTransactionState(line('.'), '* ')<CR>
